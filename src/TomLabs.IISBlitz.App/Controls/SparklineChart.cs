...
public class SparklineChart : Control
{
    ...
    public static readonly StyledProperty<string?> EmptyTextProperty =
        AvaloniaProperty.Register<SparklineChart, string?>(nameof(EmptyText), "Collecting samples…");

    public static readonly StyledProperty<string> UnitProperty =
        AvaloniaProperty.Register<SparklineChart, string>(nameof(Unit), "ms");

    public static readonly StyledProperty<string> ValueFormatProperty =
        AvaloniaProperty.Register<SparklineChart, string>(nameof(ValueFormat), "0");
    ...
    static SparklineChart()
    {
        AffectsRender<SparklineChart>(ValuesProperty, LineBrushProperty, FillBrushProperty, TextBrushProperty,
            GridBrushProperty, LineThicknessProperty, LabelProperty, EmptyTextProperty, UnitProperty, ValueFormatProperty);
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        var stats = Text($"min {min.ToString(ValueFormat, inv)} {Unit}   avg {avg.ToString(ValueFormat, inv)} {Unit}   max {max.ToString(ValueFormat, inv)} {Unit}", 10, TextBrush);
        context.DrawText(stats, new Point(chartLeft, footerY));

        var last = Text($"now {values[^1].ToString(ValueFormat, inv)} {Unit}", 10, LineBrush);
